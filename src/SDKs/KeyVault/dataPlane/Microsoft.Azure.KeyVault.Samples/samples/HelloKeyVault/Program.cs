...
namespace Sample.Microsoft.HelloKeyVault
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var nativeClientId = ConfigurationManager.AppSettings["NativeClientId"];
            ...
            userKeyVaultClient = new KeyVaultClient(new KeyVaultClient.AuthenticationCallback(
                   (authority, resource, scope) => GetUserAccessToken(authority, resource, scope, nativeClientId, userCreds)),
                   new InjectHostHeaderHttpMessageHandler());
        ...
        /// <param name="scope"> scope </param>
        /// <returns> token </returns>
        public static async Task<string> GetUserAccessToken(string authority, string resource, string scope, string nativeClientId, UserCredential userCreds)
        {
            var context = new AuthenticationContext(authority, TokenCache.DefaultShared);
            var result = await context.AcquireTokenAsync(resource, nativeClientId, userCreds).ConfigureAwait(false);
